...
io.on('connection', (socket) => {
    ...
    if (socket.userId) {
      ...
      if (user) {
        ...
        if (activeCalls.has(socket.userId)) {
          const callId = activeCalls.get(socket.userId);
          const call = await Call.findOne({ callId });
          
          if (call && (call.status === 'initiated' || call.status === 'ongoing')) {
            call.status = 'missed';
            call.endedAt = new Date();
            if (call.startedAt) {
              call.duration = Math.floor((call.endedAt - call.startedAt) / 1000);
            ...
            const otherUserId = call.caller.userId === socket.userId ? 
              call.receiver.userId : call.caller.userId;
  ...
  socket.on('sendMessage', async (messageData) => {
    try {
      ...
      const { receiverId, content, type, fileUrl } = messageData;
  ...
  socket.on('callSignal', async (data) => {
    try {
      if (!socket.userId || !data.callId || !data.signal || !data.to) {
        return;
      }
      
      // Verify call exists and user is part of it
      const call = await Call.findOne({ callId: data.callId })
        .populate('caller receiver', 'userId');
      
      if (!call || 
          (call.caller.userId !== socket.userId && call.receiver.userId !== socket.userId)) {
        return;
      }
      
      // Forward the signal to the other user
      io.to(data.to).emit('callSignal', {
        callId: data.callId,
        signal: data.signal,
        from: socket.userId
      });
      ...
      if (call.status === 'ongoing') {
        activeCalls.set(socket.userId, data.callId);
        activeCalls.set(data.to, data.callId);
      }
    } catch (error) {
  ...
  socket.on('callIceCandidate', async (data) => {
    try {
      if (!socket.userId || !data.callId || !data.candidate || !data.to) {
        return;
      }
      ...
      io.to(data.to).emit('callIceCandidate', {
        callId: data.callId,
